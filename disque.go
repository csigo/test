...
func (s *disqueService) Start() (port int, err error) {
	if err := CheckExecutable("disque-server", "disque"); err != nil {
		return 0, fmt.Errorf("Disque is not installed: %v\n", err)
	}

	s.port, err = genDisquePort(minPort, maxPort)
	if err != nil {
		return 0, fmt.Errorf("Fail to get port for disque: %v\n", err)
	}
	...
	if err := cmd.Run(); err != nil {
		return 0, fmt.Errorf("Fail to start disque server: %v\n", err)
	}
	...
	return 0, fmt.Errorf("Fail to start disque server for port: %v", s.port)
}
